package com.zjjxl.panda.arcesoft;
...
public class FaceHelper {
     ...
     */
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21, int w, int h) {
        if (faceListener != null) {
            if (ftEngine != null) {
                faceInfoList.clear();
                long ftStartTime = System.currentTimeMillis();
                int code = ftEngine.detectFaces(nv21, w, h, FaceEngine.CP_PAF_NV21, faceInfoList);
                LUtils.d(TAG, "previewSize.width===" + previewSize.width);
                LUtils.d(TAG, "previewSize.height===" + previewSize.height);

                if (code != ErrorInfo.MOK) {
                    faceListener.onFail(new Exception("ft failed,code is " + code));
                } else {
                    LUtils.i(TAG, "onPreviewFrame: ft costTime = " + (System.currentTimeMillis() - ftStartTime) + "ms");
                }
                /*
                 * 若需要多人脸搜索，删除此行代码
                 */
    ...
    public List<FacePreviewInfo> onPreviewFrame2(byte[] nv21) {
        if (faceListener != null) {
            if (ftEngine != null) {
                faceInfoList.clear();
                long ftStartTime = System.currentTimeMillis();
            ...
            if (faceListener != null && nv21Data != null) {
                if (frEngine != null) {
                    ...
                    if (frCode == ErrorInfo.MOK) {
                        //                        LUtils.i(TAG, "run: fr costTime = " + (System.currentTimeMillis() - frStartTime) + "ms");
